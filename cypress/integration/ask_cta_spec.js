...
// TEST 1 --> eNewsletter NOT CHECKED
...
describe(`Trip Internal Link CTA Test from overview page / detail button`, function () {
    it(`Clicks details CTA, fills out the form, and submits it`, function () {
        ...
        cy.get('[ctest=utm_form]').find('#recentTripPDFRequest').should('have.value', '');
        cy.get('[ctest=utm_form]').find('#recentTripPDFURL').should('have.value', '');
        cy.get('[ctest=utm_form]').find('#temp').should('have.value', '');
        //OPT IN checkboxes --> should be OFF
        cy.get('[ctest=utm_form]').find('#optInNHA').should('have.value', 'no');
        cy.get('[ctest=utm_form]').find('#optInTripDrips').should('have.value', 'no');
        cy.get('[ctest=utm_form]').find('#optInStories').should('have.value', 'no');
        cy.get('[ctest=utm_form]').find('#optInWebinar').should('have.value', 'no');
        cy.get('[ctest=utm_form]').find('#optInSurveys').should('have.value', 'no');
...
//         cy.get('[ctest=utm_form]').find('#temp').should('have.value', '');
//         //OPT IN checkboxes
//         cy.get('[ctest=utm_form]').find('#optInNHA').should('have.value', 'yes');
//         cy.get('[ctest=utm_form]').find('#optInTripDrips').should('have.value', 'yes');
//         cy.get('[ctest=utm_form]').find('#optInStories').should('have.value', 'yes');
//         cy.get('[ctest=utm_form]').find('#optInWebinar').should('have.value', 'yes');
//         cy.get('[ctest=utm_form]').find('#optInSurveys').should('have.value', 'yes');
//         //pulled from UTM codes in URL for google analytics 
//         cy.get('[ctest=utm_form]').find('#utmcampaign').should('not.have.value', '');
//         cy.get('[ctest=utm_form]').find('#utmcontent').should('not.have.value', '');
//         cy.get('[ctest=utm_form]').find('#utmmedium').should('not.have.value', '');
